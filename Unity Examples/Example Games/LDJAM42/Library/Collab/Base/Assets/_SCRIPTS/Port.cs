...
public class Port : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(transform.position, serviceRadius);
    }
    ...
    void FixedUpdate()
    {
        ...
        if (distance <= serviceRadius && distance > embarkRadius && Input.GetAxis("Horizontal") == 0 && !Input.GetKey(KeyCode.W) && !Input.GetKey(KeyCode.S))
        {
            ...
            playerRB.transform.rotation = Quaternion.Slerp(playerRB.transform.rotation, Quaternion.LookRotation(transform.position - playerRB.transform.position), maxRotate * Time.fixedDeltaTime);
            playerRB.transform.position += playerRB.transform.forward * maxSpeed * Time.fixedDeltaTime;
        }

        //port menu trigger
        if (distance <= (embarkRadius + 50))
        {
